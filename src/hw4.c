    ...
    while (!gameOver)
    {
        ...
        if (turn == 1)
        {
            ...
            else if (!begin)
            {
                ...
                else
                {
                    ...
                    {
                        width = 0;
                        height = 0;                    // just in case, height and width are set back to 0
            ...
            {
                if (buffer[0] != 'I')
                    send(conn_fd1, "E 101", 8, 0); // send_message(conn_fd1, "E 101"); //not an I request
                else
                {
            ...
            else
            {
                ...
                else if (buffer[0] == 'S')
                {
                    ...
                    else
                    {
                        ...
                        printf("ships remaining on player 2's board: %d\n\n\n", p1.ships_remaining);
                        sprintf(temp, "R %d %c", ships, result);
                        if (ships == 0)
                        {
                            gameOver = true;
                            send(conn_fd1, "R 0 H", 6, 0);
                            read(conn_fd2, buffer, BUFFER_SIZE);
        ...
        else
        {
            ...
            else if (!begin)
            {
                ...
                {
                    if (buffer[0] != 'B')
                        send(conn_fd2, "E 100", 8, 0); // send_message(conn_fd2, "E 100");
                    else
                        send(conn_fd2, "E 200", 8, 0);
                }
                else
                {
            ...
            {
                if (buffer[0] != 'I')
                    send(conn_fd2, "E 101", 8, 0); // send_message(conn_fd2, "E 101"); //not an I request
                else
                {
            ...
            else
            {
                ...
                else if (buffer[0] == 'S')
                {
                    ...
                    else
                    {
                        ...
                        printf("ships remaining on player 1's board: %d\n\n\n", p2.ships_remaining);
                        if (ships == 0)
                        {
                            gameOver = true;
                            send(conn_fd2, "R 0 H", 6, 0);
                            read(conn_fd1, buffer, BUFFER_SIZE);
                            send(conn_fd1, "H 0", 4, 0);
                            read(conn_fd2, buffer, BUFFER_SIZE);
                            send(conn_fd2, "H 1", 4, 0);
...
int processShoot(Player *pShooting, Player *pOther, char *buffer)
{
    ...
    if (value != 2)
        return 202;
    if (row < 0 || row >= pOther->height)
        return 400;
    if (col < 0 || col >= pOther->width)
        return 400;
    if (alreadyShot(pShooting, row, col))
        return 401;
    ...
    if (pOther->board[row][col] != 0)
    {
        ...
        printf("Board after hit:\n");
        printBoard(pOther);
        int num_ships = numOfShips(pOther);
        pShooting->ships_remaining = num_ships;
    }
    else
        pShooting->shot_history[pShooting->num_shots].result = 'M';
    pShooting->num_shots++;
    return 0;
}
...
int numOfShips(Player *pOther)
{
    ...
    for (int r = 0; r < pOther->height; r++)
    {
        ...
        {
            if (!one & pOther->board[r][c] == 1)
                one = true;
            if (!two & pOther->board[r][c] == 2)
                two = true;
            if (!three & pOther->board[r][c] == 3)
                three = true;
            if (!four & pOther->board[r][c] == 4)
                four = true;
            if (!five & pOther->board[r][c] == 5)
                five = true;
        }
    }
    if (!one)
...
int alreadyShot(Player *pShooting, int row, int col)
{
    if (pShooting->num_shots == 0)
        return 0;
    for (int i = 0; i < pShooting->num_shots; i++)
    {
        if (pShooting->shot_history[i].col == col && pShooting->shot_history[i].row == row)
            return 1;
    }
    return 0;
}
...
int placeShips(Player *player)
{
    ...
    for (int r = 0; r < 5; r++)
    {
        ...
        if (placeShip(player, piece_type, piece_rotation, column, row, r + 1) == 0)
            return 0;
    }
    return 1;
}
